...
export default function DealersPage() {
  ...
  const handleDelete = async (e, id) => {
    e.stopPropagation();
    if (!window.confirm('Are you sure you want to delete this dealer? Cannot delete if they have invoices.')) return;
    try {
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">Wholesale Dealers</h1>
        {canWrite && <button className="btn btn-primary" onClick={() => { setEditingId(null); setForm({ name: '', phone: '', email: '', address: '', notes: '' }); setShowForm(true); }}>+ Add Dealer</button>}
      </div>

      {showForm && (
        <div className="modal-overlay" onClick={() => setShowForm(false)}>
          <div className="modal-content card" onClick={(e) => e.stopPropagation()}>
            <h2 style={{ marginBottom: 'var(--spacing-lg)' }}>{editingId ? 'Edit' : 'New'} Dealer</h2>
        ...
        <div className="modal-overlay" onClick={() => setDetail(null)}>
          <div className="modal-content card" style={{ maxWidth: 650 }} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ color: 'var(--color-text-secondary)', marginBottom: 'var(--spacing-lg)' }}>
              Invoiced: <strong>{fmt(detail.total_invoiced)}</strong> &nbsp;•&nbsp;
              Paid: <strong>{fmt(detail.total_paid)}</strong> &nbsp;•&nbsp;
              <span style={{ color: detail.balance > 0 ? 'var(--color-danger)' : 'var(--color-success)', fontWeight: 600 }}>Balance: {fmt(detail.balance)}</span>
            </p>
            <h3 style={{ marginBottom: 'var(--spacing-sm)' }}>Invoices ({detail.invoices.length})</h3>
            <div className="table-container" style={{ maxHeight: 200, overflow: 'auto', marginBottom: 'var(--spacing-md)' }}>
              <table className="table"><thead><tr><th>#</th><th>Date</th><th>Total</th><th>Paid</th><th>Status</th></tr></thead>
            ...
            <h3 style={{ marginBottom: 'var(--spacing-sm)' }}>Payments ({detail.payments.length})</h3>
            <div className="table-container" style={{ maxHeight: 200, overflow: 'auto' }}>
              <table className="table"><thead><tr><th>Date</th><th>Method</th><th>Amount</th><th>Ref</th></tr></thead>
      ...
      {loading ? <div className="loading-screen"><div className="spinner" /></div> : (
        <div className="table-container">
          <table className="table"><thead><tr><th>Name</th><th>Phone</th><th>Invoiced</th><th>Paid</th><th>Balance</th><th>Actions</th></tr></thead>
            <tbody>{dealers.map((d) => (
                ...
                <td>
                  {canWrite && (
                    <div style={{ display: 'flex', gap: 'var(--spacing-sm)' }}>
                      <button className="btn btn-sm btn-secondary" onClick={(e) => { e.stopPropagation(); setForm({ name: d.name, phone: d.phone || '', email: d.email || '', address: d.address || '', notes: d.notes || '' }); setEditingId(d.id); setShowForm(true); }}>Edit</button>
